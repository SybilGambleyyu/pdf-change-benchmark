...
fixture pairs, passed 90 tests and Ruff, and scored the PDFFence 1.23.0 source
candidate exactly through its process-bound command-line executable: 160 of 160
pairs passed.

The new terminal-footer semantic-signature control begins with a field-root
signature and an ordinary later incremental revision, so both sides retain a
well-formed range at the historical signing footer and neither reaches the
current physical file end. The candidate appends unlinked raw bytes after the
final PDF footer rather than adding a cross-reference section or trailer. Its
truth requires only the established own-revision inventory event and PFP013,
along with stored-byte evidence; PFP014 has no separate event because the
own-revision endpoint prerequisite itself changed. The held PDFFence 1.22.0
wheel reports stored-byte evidence alone, so it does not satisfy this new
contract. The fixture does not claim signature validity or publish a tail,
object reference, revision boundary, offset, range, signature content,
...
and all 160 process-bound score cases. Dependency auditing found no known
vulnerabilities; the local distributions were correctly skipped because they
are not on PyPI.

## 1.22.0 source-candidate integration evidence

On 2026-08-06, the 1.22.0 fixture candidate rebuilt and verified all 159
fixture pairs, passed 89 tests and Ruff, and scored the PDFFence 1.22.0 source
candidate exactly through its process-bound command-line executable: 159 of
159 pairs passed.

The new semantic-signature control gives both sides a later incremental update,
so neither ByteRange reaches its current physical file end. Both ranges still
terminate at their own signing-revision footers. The baseline's excluded gap
is exactly its direct hexadecimal `/Contents` token; the candidate's gap begins
one byte earlier. Only the distinct historical Contents inventory event and
PFP014 are expected. PFP009, PFP012, and PFP013 remain quiet; PFP010 and PFP011
are deliberately current-file gates and are outside this historical contract.
The held PDFFence 1.21.0 wheel reports stored-byte evidence alone for this
pair, so it does not satisfy the new historical-Contents contract. The fixture
does not claim signature validity or publish an object reference, revision
boundary, offset, signature content, certificate, digest, or trust result.
...
verification, and all 159 process-bound score cases. Dependency auditing found
...
fixture pairs with pypdf 6.15.0. Its 85 tests and Ruff checks passed. It scored
the PDFFence 1.18.0 source candidate through its process-bound command-line
executable: 155 of 155 pairs passed.

The new signature control makes its baseline and candidate both retain the
same well-formed semantic ByteRange before their current physical file ends.
The candidate adds a distinct incremental revision, but public coverage counts
stay fixed; its truth requires PFP010 and no coverage-change event. It does not
claim a signature is invalid or publish an offset, signature value, certificate,
digest, or trust result. The PDFFence 1.17 PFP009 rule emits no finding for the
same already-stale pair, because it is correctly a regression-only rule.

Two fixed-timestamp builds produced byte-identical wheel and source-archive
files, and all eight PDFFence and PDFCAB artifacts passed Twine metadata
checks. Fresh paired wheel installs on Python 3.12 and 3.13, plus a clean
paired Python 3.12 source-archive install, passed `pip check`, fixture
verification, and the complete 155-pair score. Dependency auditing found no
